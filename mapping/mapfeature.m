...
if(is_string(feature)); feature=cellstr(feature); end
...
% check opt
hopt=cell(nf,1);
if(is_string(opt)); opt=cellstr(opt); end
if(iscell(opt))
    % encapsulate {'...' ## ##}
    if(numel(opt)==3 && is_string(opt{1}) && isnumeric(opt{2}) ...
            && isnumeric(opt{3}))
        opt={opt};
    end
    ...
    for i=1:numel(opt)
        ...
        if(numel(opt{i})==3 && is_string(opt{i}{1}) ...
