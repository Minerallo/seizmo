function [mout]=prem_perfect(varargin)
%PREM_PERFECT    Returns the PREM model
%
%    Usage:    model=prem_perfect()
%              model=prem_perfect('refperiod',period)
%              model=prem_perfect('anisotropic',true|false)
%              model=prem_perfect('crust',true|false)
%              model=prem_perfect('ocean',true|false)
%              model=prem_perfect('depths',depths)
%              model=prem_perfect('spvw',nsamples)
%              model=prem_perfect('maxdepth',maxdepth)
%
%    Description:
%     MODEL=PREM_PERFECT() returns the Preliminary Reference Earth Model
%     (PREM) at a reference period of 1Hz sampled once per vertical
...
%
%     MODEL=PREM_PERFECT('REFPERIOD',PERIOD) changes the reference period
...
%
%     MODEL=PREM_PERFECT('DEPTHS',DEPTHS) indicates specific depths (in km)
%     to return the PREM model parameters at.  The default is [] (empty
%     matrix) and will instead use the SPVW option to sample the PREM
%     model.  Note that depths specified at discontinuities will return
%     values for both sides.
%
%     MODEL=PREM_PERFECT('SPVW',NSAMPLES) number of samples (or knots if
%     you prefer) per vertical wavelength to sample the PREM model at.  The
%     default is 1.  This option is ignored if the DEPTHS option is
...
%
%     MODEL=PREM_PERFECT('MAXDEPTH',MAXDEPTH) is the maximum depth to
...
%     - Missing pressure values and bulk sound speed derivatives.
%     - It is slow, I know.  The implementation isn't optimal for speed.
%
%    Examples:
%     % only sample to 5000km depth to exclude the inner-core
%     model=prem_perfect('m',5000);
%     plot1dmodel(model);
%
%    See also: PREM, PREM2_PERFECT, IASP91, AK135, CMB_1DMODEL_LIBRARY

%     Version History:
%        Jan. 17, 2010 - initial version
%        Aug. 17, 2010 - drop all the custom CMB codes, add documentation,
%                        add option checking, fixed up dispersion, bulk
